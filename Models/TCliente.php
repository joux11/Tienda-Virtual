...
trait TCliente{
	...
	public function insertDetalleTemp(array $pedido){
		...
		if(empty($request)){
			foreach ($productos as $producto) {
				$query_insert  = "INSERT INTO detalle_temp(personaid,idproducto,precio,cantidad,transaccionid) 
								  VALUES(?,?,?,?,?)";
	...
	public function insertDetalle(int $idpedido, int $productoid, float $precio, int $cantidad){
		$this->con = new Mysql();
		$query_insert  = "INSERT INTO detallepedido(pedidoid,productoid,precio,cantidad) 
							  VALUES(?,?,?,?)";
	...
	public function getPedido(int $idpedido){
		...
		$requestPedido = $this->con->get($sql);
		if(count($requestPedido) > 0){
			$sql_detalle = "SELECT p.idproducto,
									...
									";
			$requestProductos = $this->con->getall($sql_detalle);
